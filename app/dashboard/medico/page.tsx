...
type MedicPanel = "intercambio" | "reportar" | "inventario" | "distribucion";
...
export default function MedicDashboardPage() {
  ...
  const inventoryPageCount = Math.max(
    1,
    Math.ceil(hospitalResources.length / inventoryPageSize),
  );
  const safeInventoryPage = Math.min(inventoryPage, inventoryPageCount);
  const paginatedResources = hospitalResources.slice(
    ...
    <AppShell eyebrow="Rol medico" title="Centro de solicitudes medicas">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[230px_1fr]">
        <aside className="rounded-[18px] border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-6">
          ...
          <nav className="grid gap-2 text-xs font-black">
            ...
            <button
              className={menuButtonClass("inventario")}
              onClick={() => setActivePanel("inventario")}
              type="button"
            >
              Ver inventario
            </button>
      ...
      {activePanel === "inventario" ? (
        <InventoryTablePanel
          ...
          surplusCount={surplusResources.length}
          totalCount={hospitalResources.length}
        />
      ) : null}

      <section
        className={`rounded-[18px] border border-slate-200 bg-white p-5 shadow-sm ${
          activePanel === "distribucion" || activePanel === "inventario"
            ? "hidden"
            : ""
        }`}
      >
    ...
    <section className="rounded-[18px] border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col justify-between gap-4 border-b border-slate-100 pb-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            Consulta todos los insumos reportados por el centro hospitalario.
          </p>
        </div>
      ...
      <div className="mt-5 overflow-hidden rounded-[14px] border border-slate-200">
        <div className="overflow-x-auto">
          <table className="min-w-[760px] w-full border-collapse text-left text-sm">
            <thead className="bg-slate-50 text-[0.65rem] font-black uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-4 py-3">Insumo / medicamento</th>
                <th className="px-4 py-3">Tipo</th>
                <th className="px-4 py-3">Obtenido</th>
                <th className="px-4 py-3">Meta requerida</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {resources.map((resource) => {
                const isMissing = resource.type === "Faltante";

                return (
                  <tr className="bg-white" key={`${resource.item}-${resource.updatedAt}`}>
        ...
        {!resources.length ? (
          <p className="border-t border-slate-100 bg-slate-50 p-6 text-center text-sm text-slate-500">
            No hay insumos registrados en este hospital todavia.
          </p>
        ) : null}
      </div>
      ...
      <div className="mt-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <p className="text-xs font-bold text-slate-500">
          Mostrando {firstRow}-{lastRow} de {totalCount} registros
        </p>
        <div className="flex items-center gap-2">
          <button
            className="rounded-[10px] border border-slate-200 px-4 py-2 text-xs font-black text-slate-700 transition hover:border-blue-200 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-40"
          ...
          <span className="rounded-[10px] bg-blue-50 px-4 py-2 text-xs font-black text-blue-700">
